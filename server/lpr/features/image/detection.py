...
def detect_image(
    ...
    if detect_car:
        ...
        if not car_detections:
            raise LookupError("Car detection could not find any car.")
        ...
        if not regions:
            raise LookupError("Car detection found no valid car regions.")
    else:
        regions = [
            {
                "carIndex": None,
    ...
    for region in regions:
        ...
        ocr_result = detect_text(ocr_input, preprocess=preprocess_ocr)
    ...
    if not results:
        raise LookupError("No valid detections were found for OCR.")
